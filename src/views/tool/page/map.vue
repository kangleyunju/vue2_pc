<template>
	<div class="mapContainer" v-loading="loading" element-loading-text="拼命加载中">
		<div class="infoBox">
			 <el-select v-model="locationId" placeholder="请输入地点" :remote-method="inputWord" clearable filterable remote size="medium" @clear="clear" @change="chooseLocation">
	...
	export default {
		...
		methods: {
			...
			inputWord(keyword) {
			  ...
			  this.$jsonp(`https://apis.map.qq.com/ws/place/v1/suggestion?keyword=${this.keyword}&key=${this.key}&output=jsonp&region=${this.ad_info.city}`)
			  .then(res => {
